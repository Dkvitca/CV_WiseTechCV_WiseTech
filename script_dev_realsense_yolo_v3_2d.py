...
class draw():
    ...
    def update_frame(self,classId, conf, left, top, right, bottom, x ,y,distance):
        self.min_distance = distance 
...
def process_detection(frame, outs):
    ...
    for i in indices:
        ...
        if drawing.x == 0 :
            ...
            drawing.update_frame(classIds[i], confidences[i], left, top, left+width, top+height,x,y,INFINITY)
            drawing.drawPredicted(drawing.class_id, drawing.conf,drawing.left,drawing.top,drawing.right,drawing.bottom,frame,drawing.x,drawing.y)
            
        elif (drawing.min_distance >= distance and distance != 0) :
            drawing.update_frame(classIds[i], confidences[i], left, top, left+width, top+height,x,y,distance)
            drawing.drawPredicted(drawing.class_id, drawing.conf,drawing.left,drawing.top,drawing.right,drawing.bottom,frame,drawing.x,drawing.y)
        else:
            return
